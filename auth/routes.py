...
# ────────────────────────────────────────────────
# Helper for Broken Access Control / IDOR logging
# ────────────────────────────────────────────────
def log_broken_access_control(actor_id, actor_username, target_id, action_type, details, ip_address):
    """
    Logs a Broken Access Control / IDOR attempt:
    - Calls log_monitor_action
    - Inserts a monitor message for the affected user
    """
...
# ────────────────────────────────────────────────
# RESET PASSWORD – monitor token usage
# ────────────────────────────────────────────────
...
def reset_password(token):
    ...
    try:
        ...
        # Log for monitoring potential token abuse
        log_broken_access_control(
            actor_id=user_id,
            actor_username=user_data['username'],
            target_id=user_id,
            action_type="password_reset_success",
            details="Password reset completed via token (monitoring for potential abuse)",
            ip_address=request.remote_addr
        )
...
def change_password():
    ...
    try:
        ...
        if str(target_user_id) != str(current_user.id):
            flash("Attempted to change another user's password! Logged.", "warning")
            log_broken_access_control(
                actor_id=current_user.id,
                actor_username=current_user.username,
                target_id=target_user_id,
                action_type="change_password_attempt",
                details=f"User {current_user.username} attempted to change password of user {target_user_id} (Broken Access Control)",
                ip_address=request.remote_addr
            )
            return redirect(url_for("auth.change_password"))
...
def change_email():
    ...
    try:
        ...
        # Broken Access Control / IDOR
        if str(target_user_id) != str(current_user.id):
            flash("Attempted to change another user's email! Logged.", "warning")
            log_broken_access_control(
                actor_id=current_user.id,
                actor_username=current_user.username,
                target_id=target_user_id,
                action_type="change_email_attempt",
                details=f"User {current_user.username} attempted to change email of user {target_user_id} (Broken Access Control)",
                ip_address=request.remote_addr
            )
