...
# ────────────────────────────────────────────────
# HOME / LISTINGS OVERVIEW (no direct exploit here, but visible to all)
# ────────────────────────────────────────────────
@vuln_bp.route("/")
@login_required
def home():
...
def profile(user_id):
    ...
    try:
        ...
        # Log ONLY if viewing someone else's profile (IDOR / privacy leak)
...
@login_required
def edit_profile():
    # Vulnerable: allow tampering user_id via form
    ...
    try:
        ...
        role = request.form.get("role", user_data['role'])  # MASS ASSIGNMENT VULN

        # VULN: no ownership check – can edit any profile by tampering user_id
        cursor.execute("""
            UPDATE users
        ...
        # Log ONLY if target is not self (IDOR / mass assignment attempt)
...
def listing_detail(listing_id):
    ...
    try:
        ...
        # Log only if viewing someone else's listing (potential IDOR)
        if not is_owner:
            log_monitor_action(
                user_id=current_user.id,
                username=current_user.username,
...
# ────────────────────────────────────────────────
# CREATE NEW LISTING – Mass Assignment VULN (owner_id tampering)
# ────────────────────────────────────────────────
...
def new_listing():
    ...
    try:
        ...
        # Log only if owner_id was tampered (mass assignment)
...
def edit_listing(listing_id):
    ...
    try:
        ...
        # Log only if owner_id was tampered or status changed suspiciously
        if owner_id != current_user.id:
            log_monitor_action(
                user_id=current_user.id,
                username=current_user.username,
...
def delete_listing(listing_id):
    ...
    try:
        ...
        # Log delete action (always log deletes – high impact)
        log_monitor_action(
            user_id=current_user.id,
            username=current_user.username,
...
@login_required
def admin():
    # VULN: no role check – any logged-in user can access
    conn = get_db_connection()
    try:
        cursor = get_cursor(conn)
        ...
        # Log admin access (high impact – always log)
        log_monitor_action(
            user_id=current_user.id,
            username=current_user.username,
